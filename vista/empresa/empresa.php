<body>
  <!-- Core Stylesheet -->
  <head>
  
  <link href="../estilos/principal.css" rel="stylesheet" >
   <link  href="../estilos/style.css.map" rel="stylesheet">
   <link rel="stylesheet" href="../../css/pag-print.css">
   <link rel="stylesheet" href="../../css/principal.css">
   <link rel="stylesheet" href="../../css/empresa.css">
   <link rel="stylesheet" href="../../css/_sections.scss">
   </head>
   <div class="nav">

</div>
<header id="header">
    <div class="container">
      <div id="logo" class="pull-left">
        <a href="#hero"><img src="img/logo.png" alt="" title="" /></img></a>
        <!-- Descomenta abajo si prefieres usar una imagen de texto -->
        <!--<h1><a href="#hero">Encabezado 1</a></h1>-->
      </div>

      <nav id="nav-menu-container">
        <ul class="nav-menu">
          <li class="menu-active"><a href="../../index.php">Inicio</a></li>
          <li><a href="#about">La Empresa</a></li>
          <li><a href="#services">Farmacovigilancias</a></li>
          <li><a href="#portfolio">Vademecum</a></li>
          <li class="menu-has-children"><a href="">Productos</a>
            <ul>
              <li><a href="#">producto 1</a></li>
              <li class="menu-has-children"><a href="#">producto 2</a>
                <ul>
                  <li><a href="#">producto 1</a></li>
                  <li><a href="#">producto 2</a></li>
                  <li><a href="#">producto 3</a></li>
                </ul>
              </li>
              <li><a href="#">producto 3</a></li>
              <li><a href="#">producto 4</a></li>
              <li><a href="#">producto 5</a></li>
            </ul>
          </li>
          <li><a href="#contact">Contactos</a></li>
        </ul>
      </nav>
      <!-- #nav-menu-container -->
    </div>
</header>
<div class="container">
  <div class="row">
    <!-- ##### Preloader ##### -->
    <section class="about-us" id="about-us">
  
    <h1>NOSOTROS</h1>
   <div class="column-one">
      <div class="circle-one"></div>
      <h2>MISION</h2>
      <p>“Laboratorios Alfa es una empresa industrial farmacéutica que tiene el propósito de desarrollar, fabricar y comercializar medicamentos accesibles, efectivos y seguros, que contribuyan a mantener y restablecer la salud de la población boliviana.”
      </p>

    </div>
  
      <div class="column-two">
  
        <div class="circle-two"></div>
    
          <h2>MISION</h2>
          <p>“Queremos ser, en el año 2025, la empresa industrial farmacéutica boliviana, líder de excelencia, que se adapta permanentemente al cambio para satisfacer las necesidades de nuestros clientes.”
</p>
    
      </div>
  
  
      <div class="column-three">
  
        <div class="circle-three"></div>
    
        <h2>VALORES</h2>
        <p>El trabajo en equipo, La fuerza de la verdad.
La riqueza del conocimiento.
La responsabilidad.
El entusiasmo.
La lealtad 
El respeto.</p>
    
    </div>  
    

  </section>
 
  </div>
</div>
<section id="testimonials">
    <div class="container wow fadeInUp">
      <div class="row">
        <div class="col-md-12">
          <h3 class="section-title">HISTORIA</h3>
          <div class="section-title-divider"></div>
          <p class="section-description">TEXTO AQUI</p>
        </div>
      </div>

      <div class="row">
        <div class="col-md-3">
          <div class="profile">
            <div class="pic1"><img src="../../css/img/Imagen4.jpg" alt=""></div>
            <h4>Dr. Santiago Ferrer Alsina</h4>
            <span>Lawless Inc</span>
          </div>
        </div>
        <div class="col-md-9">
          <div class="quote">
            <b><img src="img/quote_sign_left.png" alt=""></b> Laboratorios ALFA S.A., nace como empresa unipersonal en Marzo de 1965, fundada por el Dr. Santiago Ferrer Alsina, con el propósito y la visión emprendedora de transformarla posteriormente en una empresa familiar de responsabilidad limitada.
 <small><img src="img/quote_sign_right.png" alt=""></small>
          </div>
        </div>
      </div>

      <div class="row">
        <div class="col-md-9">
          <div class="quote">
            <b><img src="" alt=""></b> El año 1977, la empresa se transforma en Sociedad de Responsabilidad Limitada (S.R.L.) con la integración como socios accionistas de la esposa y los dos hijos del fundador.
 <small><img src="img/quote_sign_right.png" alt=""></small>
          </div>
        </div>
        <div class="col-md-3">
          <div class="profile">
            <div class="pic"><img src="../../css/img/Imagen5.jpg" alt=""></div>
            <h4>Sara Wilsson</h4>
            <span>Odeo Inc</span>
          </div>
        </div>
      </div>
      <div class="row">
      <div class="col-md-3">
          <div class="profile">
            <div class="pic"><img src="../../css/img/Imagen6.jpg" alt=""></div>
            <h4>Sara Wilsson</h4>
            <span>Odeo Inc</span>
          </div>
        </div>
        <div class="col-md-9">
          <div class="quote">
            <b><img src="img/quote_sign_left.png" alt=""></b>  A principios de la década de los 80's, se trasladan las instalaciones a su nueva planta ubicada en la Plaza Uyuni, donde desarrolla actualmente sus actividades. <small><img src="img/quote_sign_right.png" alt=""></small>
          </div>
        </div>
        
      </div>
      <div class="row">
        <div class="col-md-9">
          <div class="quote">
            <b><img src="" alt=""></b>  En Abril de 2015 se incorporan nuevos socios con nuevos aportes de capital y “know-how”, iniciando la transformación de S.R.L. a S.A. Asimismo, se constituye en Asunción del Paraguay la Subsidiaria Pharmanest S.A.
 <small><img src="img/quote_sign_right.png" alt=""></small>
          </div>
        </div>
        <div class="col-md-3">
          <div class="profile">
            <div class="pic"><img src="../../css/img/Imagen7.jpg" alt=""></div>
            <h4>Sara Wilsson</h4>
            <span>Odeo Inc</span>
          </div>
        </div>
      </div>

    </div>
  </section>

   
   

    <!-- ##### Footer Area Start ##### -->
  <footer class="footer-area">
        <div  class="main-footer-area section-padding-100-0">
            <div class="container">
                <div class="row">
                    
                </div>
            </div>
        </div>
        <div class="bottom-footer-area">
            <div class="container">
                <div class="row" >
                    <div class="col-12" >
                        <p style="color: white;"><!-- Link back to Colorlib can't be removed. Template is licensed under CC BY 3.0. -->
                        Copyright derechos reservados 2022
<!-- Link back to Colorlib can't be removed. Template is licensed under CC BY 3.0. --></p>
                    </div>
                </div>
            </div>
        </div>
</footer>
    <!-- ##### Footer Area Start ##### -->

    <!-- ##### All Javascript Script ##### -->
    <!-- jQuery-2.2.4 js -->
    <script src="js/js/jquery/jquery-2.2.4.min.js"></script>
    <!-- Popper js -->
    <script src="js/js/bootstrap/popper.min.js"></script>
    <!-- Bootstrap js -->
    <script src="js/js/bootstrap/bootstrap.min.js"></script>
    <!-- All Plugins js -->
    <script src="js/js/plugins/plugins.js"></script>
    <!-- Active js -->
    <script src="js/js/active.js"></script>
    <script src="js/prueba.js"></script>
    <script src="js/localizacion.js"></script>

</body>
